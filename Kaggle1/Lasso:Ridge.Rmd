...
train <- select(train, -last_review, -minimum_nights, -days_since_last_review)
test  <- select(test,  -last_review, -minimum_nights, -days_since_last_review)
```
...
ridge_recipe <- recipe(log_price ~ ., data = t_train) %>%
...
airbnb_folds <- vfold_cv(t_train, v = 10)
...
ridge_final_fit <- ridge_final_wf %>% fit(t_train)

rmse_ridge <- augment( ridge_final_fit, new_data = t_test ) %>%
  dplyr::mutate(
    price_estimate2 = exp(.pred)
  ) %>%
  dplyr::mutate(
    price_estimate_corrected2 = expm1(.pred)
  ) %>%
  yardstick::rmse(truth = price, estimate = price_estimate_corrected2)

rmse_ridge$.estimate

```

## fit submission ridge
```{r}
log_preds_test_ridge <- predict(ridge_final_fit, new_data = test)

# 2. Back-transform the predictions and format for submission
final_submission_ridge <- test %>%
  dplyr::select(ID) %>%
  dplyr::bind_cols(log_preds_test_ridge) %>%
  dplyr::mutate(
    price = expm1(.pred)) %>%
  dplyr::select(ID, price)

# 3. View the final output
print((final_submission_ridge))

# 4. Export for submission
write.csv(final_submission_ridge, "test_submission_ridge.csv", row.names = FALSE)
```
############################################################
# 14.) LASSO REGRESSION
############################################################
