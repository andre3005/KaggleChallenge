...
train_raw <- read.csv("mlai-2025-regression-challenge/train.csv")
test_raw  <- read.csv("mlai-2025-regression-challenge/test.csv")
...
# ----- 10. Temporal Seasonality Feature (Extended Summer Season) -----
```{r}
# Reviews are often written days or even weeks after a stay.
# In the dataset, September shows the highest number of reviews,
# likely reflecting delayed reviews from summer guests.
# Therefore, the high (summer) season is extended to June–September
# to better capture actual booking activity and demand patterns.
...
# Define summer season as June–September (months 6–9)
train$is_summer_season <- ifelse(train$review_month %in% c(6, 7, 8, 9), 1, 0)
test$is_summer_season  <- ifelse(test$review_month %in% c(6, 7, 8, 9), 1, 0)

# Replace NAs (listings without reviews) with 0 = not in summer season / inactive
train$review_month[is.na(train$review_month)] <- 0
test$review_month[is.na(test$review_month)]   <- 0
train$is_summer_season[is.na(train$is_summer_season)] <- 0
test$is_summer_season[is.na(test$is_summer_season)]   <- 0

# Quick check to verify transformation
cat("\nSummary of seasonal features (with extended summer definition):\n")
summary(train[, c("review_month", "is_summer_season")])

# Optional visualization to confirm monthly pattern
ggplot(train, aes(x = review_month, fill = factor(is_summer_season))) +
  geom_bar() +
  labs(
    title = "Distribution of Last Review by Month",
    x = "Review Month",
    y = "Listing Count",
    fill = "Summer Season (June–September)"
  ) +
  theme_minimal()
```
...
```{r}
# The original date variable is no longer needed
train <- select(train, -last_review)
test  <- select(test,  -last_review)
train <- select(train, -minimum_nights, -days_since_last_review)
test  <- select(test,  -minimum_nights, -days_since_last_review)
```
